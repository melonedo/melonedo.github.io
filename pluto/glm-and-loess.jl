...
# ╔═╡ bc465c99-6cfb-4caa-9282-2e5054eeb0b2
begin
	using Plots,GLM,DataFrames, Statistics,Loess
	import PlotlyJS
	plotlyjs()
end

# ╔═╡ 4014b7f4-43be-49cb-9f83-d222fcfea16a
data = [
    1 2 3 4
    1 4-1 9+1 16
]

# ╔═╡ 3977a7b9-1e4c-439c-938d-f1fcc57bafef
data[1,:]

# ╔═╡ eb68e5c1-a600-4d5a-9885-af562368ce2a
df = DataFrame(X=data[1,:], Y=vec(data[2,:]))

# ╔═╡ d95a334c-da95-47c5-83c9-2aaabc6f3127
ols = lm(@formula(Y ~ X^2), df)

# ╔═╡ 6aa082c3-d35e-46a3-9f14-4d88b35234ee
x1 = 0:.1:5

# ╔═╡ 0e1d491b-6656-4cbe-b28b-422a9908d543
y1 = GLM.predict(ols, DataFrame(X=x1))

# ╔═╡ 6798f602-0d9b-4945-9a10-5180bc4465a8
let
	scatter(data[1,:], data[2,:])
	plot!(x1, y1)
	savefig("plot-glmZ.html")
end

# ╔═╡ a51f0b3f-09c0-4e17-9b18-b66c49e7f239
model = loess(data[1,:], data[2,:], span=.7)

# ╔═╡ c811a802-58b5-4153-ac55-c1f5e55cafce
x2 = range(extrema(data[1,:])...; step = 0.1)

# ╔═╡ 508e2db8-26f6-4c12-a8fc-343dfef26715
y2 = Loess.predict(model, x2)

# ╔═╡ d2145bfe-6876-4544-8429-be63b026aa88
let
	scatter(data[1,:], data[2,:])
	plot!(x2, y2)
	savefig("plot-loess.html")
end
